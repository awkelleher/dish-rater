...
export default async function BestOfFoodPage({ params }: PageProps) {
  const { category } = await params;
  const decodedCategory = decodeURIComponent(category);
  const supabase = await createClient();

  // Fetch top 10 dishes of this category across all of JC, sorted by average rating
      ...
      <div className="container mx-auto px-6 py-8 max-w-6xl">
        ...
        <div className="mb-8 md:mb-12 border-b-4 border-foreground pb-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-foreground mt-4 mb-3 uppercase tracking-tight">
            Top 10 {decodedCategory} in Jersey City
          </h1>
          <p className="text-lg md:text-xl text-muted-foreground uppercase tracking-wide font-bold">
            The highest-rated {decodedCategory.toLowerCase()} across all neighborhoods
          </p>
        </div>
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        ...
                        {restaurant?.neighborhood && (
                          <p className="text-sm text-muted-foreground mt-2 font-bold uppercase tracking-wide">
                            📍 {restaurant.neighborhood}
                          </p>
                        )}
                      </div>
